...
{
    public sealed class VoxelBlueprintPartBoxCutWindow : EditorWindow
    {
        ...
        {
            public List<VoxelBlueprintPartBoxCut.RejectedCells> rejected;
            public List<VoxelBlueprintPartBoxCut.RejectedCells> claimed;
        }
        ...
        string _status = "";
        readonly int[] _counts = new int[VoxelBlueprintPartBoxCut.PartCount];
        int _ownerStamp;
        ...
        int _scanY;
        int _scanZ;
        VoxelBlueprintPartBoxCut.Box _buildBox;
        bool _buildDest;
        bool _buildLeftover;
        ...
        internal static Color ColorOf(int i) => PartColors[i];

        [MenuItem("TuZhi/Unit/Rotate V06 Armor Yaw 180 And Recut")]
        static void MenuRotateV06Yaw180AndRecut()
        {
            if (!Application.isBatchMode
                && !EditorUtility.DisplayDialog(
                    "V06 绕 Y 转 180°",
                    "把 V06 盔甲全身图纸绕 Y 转 180°，切割盒一起转，然后重切分件并拷到 VR3。不要连点两次。",
                    "转",
                    "取消"))
                return;
            string status = VoxelBlueprintPartBoxCutUtil.RotateV06Yaw180AndRecut();
        ...
        [MenuItem("TuZhi/Unit/Aigei Armor Box Cut")]
        static void Open()
        {
            VoxelBlueprintPartBoxCutWindow window = GetWindow<VoxelBlueprintPartBoxCutWindow>("Armor Box Cut");
            window._cut = VoxelBlueprintPartBoxCutUtil.LoadOrCreateCut();
            window.SeedIfEmpty();
            window.Show();
        }

        public static void OpenForFullBody(VoxelBlueprintAsset source)
        {
            string title = source != null ? "Box Cut: " + source.name : "Voxel Box Cut";
            VoxelBlueprintPartBoxCutWindow window = GetWindow<VoxelBlueprintPartBoxCutWindow>(title);
            window._cut = VoxelBlueprintPartBoxCutUtil.LoadOrCreateCutFor(source);
            window.SeedIfEmpty();
            window.Show();
            window.Focus();
        }
        ...
        void OnEnable()
        {
            ...
            if (_cut == null)
                _cut = VoxelBlueprintPartBoxCutUtil.LoadLastOrDefault();
            SeedIfEmpty();
        ...
        void OnDisable()
        {
            ...
            if (_cut != null)
            {
                if (VoxelBlueprintPartBoxCutUtil.HasStubBoxes(_cut))
                    VoxelBlueprintPartBoxCutUtil.SeedBoxesFromCurrentParts(_cut);
                else
                    VoxelBlueprintPartBoxCutUtil.Persist(_cut);
            }
            _preview?.Cleanup();
            _preview = null;
        ...
        void OnGUI()
        {
            ...
            using (new EditorGUI.DisabledScope(_cut == null || _cut.source == null))
            {
                if (GUILayout.Button("开始切割", GUILayout.Height(28f)))
                    RunSlice();
                if (GUILayout.Button("V06 全身绕 Y 转 180° 并重切", GUILayout.Height(28f)))
                    RunV06Yaw180();
            }
            if (_cut != null && _leftover > 0)
                EditorGUILayout.HelpBox("还有 " + _leftover + " 格没进任何盒，截取时不会写入这些格。", MessageType.Warning);
            EditorGUILayout.HelpBox(
                "移动：WASD 平移，R 上 F 下。键位锁世界轴，不跟镜头转：W 后(-Z) S 正前(+Z) D 人左(-X) A 人右(+X)。缩放：WASD/RF 往该方向加大，Shift 减小。预览边上的字母标的是世界方向。预览中键旋转、右键平移。右侧剥层露出内部再画。左键点/框选：勾选铅笔=收回归属，不勾橡皮=放弃归属。回滚/重做或 Ctrl+Z / Ctrl+Y 只撤销铅笔橡皮。清空=去掉全部铅笔橡皮改动（切盒不动）。四肢勾选镜像=勾在「要生成的那一侧」：该侧图纸=对侧图纸按全身中线 X 翻转，不用本侧原格子。例如左手勾镜像，就只切右手，左手由右手对称生成。多图纸罩住同一格：先看谁主动放弃，再按四肢>头>髋>胸。没进盒的格子可以不切。",
                MessageType.Info);
            EditorGUI.BeginChangeCheck();
            _cut = (VoxelBlueprintPartBoxCut)EditorGUILayout.ObjectField("Cut", _cut, typeof(VoxelBlueprintPartBoxCut), false);
            if (_cut == null)
                return;
            bool lrDone = _cut.limbViewerLrSwapDone;
            _cut.EnsureBoxes();
            if (!lrDone && _cut.limbViewerLrSwapDone)
                EditorUtility.SetDirty(_cut);
            _cut.source = (VoxelBlueprintAsset)EditorGUILayout.ObjectField("Full body", _cut.source, typeof(VoxelBlueprintAsset), false);
            using (new EditorGUILayout.HorizontalScope())
            {
                if (GUILayout.Button("从当前切件填盒"))
                    VoxelBlueprintPartBoxCutUtil.SeedBoxesFromCurrentParts(_cut);
            }
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                using (new EditorGUILayout.VerticalScope(GUILayout.ExpandWidth(true)))
                {
                    ...
                    VoxelBlueprintPartBoxCut.Box box = _cut.boxes[edit];
                    box.EnsureOriented();
                    if (edit != _selected)
                        EditorGUILayout.HelpBox("该部位由对侧镜像生成，下面改的是对侧切盒。", MessageType.Info);
                    EditorGUI.BeginChangeCheck();
                    if (_vecPasteSlot == 4)
                    {
        ...
        void RunSlice()
        {
            if (_cut == null || _cut.source == null)
                return;
            _status = VoxelBlueprintPartBoxCutUtil.Slice(_cut);
        ...
        void RunV06Yaw180()
        {
            if (!EditorUtility.DisplayDialog(
                    "V06 绕 Y 转 180°",
                    "把 V06 盔甲全身图纸绕 Y 转 180°，切割盒一起转，然后重切分件。不要连点两次。",
                    "转",
                    "取消"))
                return;
            _status = VoxelBlueprintPartBoxCutUtil.RotateV06Yaw180AndRecut();
            if (_status.StartsWith("FAIL"))
                Debug.LogError(_status);
            else
                Debug.Log(_status);
            _cut = VoxelBlueprintPartBoxCutUtil.LoadOrCreateCutFor(
                AssetDatabase.LoadAssetAtPath<VoxelBlueprintAsset>(
                    "Assets/VoxelBlueprints/Good/AigeiFantasyRpgArmor_VoxelV06/PmUnitVoxelFullBodyStyle_AigeiFantasyRpgArmor_VoxelV06.asset"));
        }
        ...
        void DrawPartSidebar()
        {
            using (new EditorGUILayout.VerticalScope(GUILayout.Width(248f), GUILayout.ExpandHeight(true)))
            {
                ...
                for (int i = 0; i < VoxelBlueprintPartBoxCut.PartCount; i++)
                {
                    ...
                    GUI.backgroundColor = PartColors[i];
                    using (new EditorGUILayout.HorizontalScope())
                    {
                        if (GUILayout.Toggle(_selected == i, PartButtonLabel(i), "Button", GUILayout.Width(140f)))
                            _selected = i;
                        EditorGUILayout.LabelField(_counts[i].ToString(), GUILayout.Width(40f));
                        string limbLabel = _cut.boxes[i].label;
                        if (VoxelBlueprintPartBoxCut.ArmMate(limbLabel) != null)
                        {
                            VoxelBlueprintPartBoxCut.Box row = _cut.boxes[i];
                            bool next = GUILayout.Toggle(row.mirror, "镜像", GUILayout.Width(48f));
                            if (next != row.mirror)
                            {
                                Undo.RecordObject(_cut, "Part Mirror");
                                row.mirror = next;
                                _cut.boxes[i] = row;
                                if (next)
                                {
                                    int mate = _cut.IndexOf(VoxelBlueprintPartBoxCut.ArmMate(limbLabel));
                                    if (mate >= 0)
                                    {
                                        VoxelBlueprintPartBoxCut.Box other = _cut.boxes[mate];
                                        other.mirror = false;
                                        _cut.boxes[mate] = other;
                                    }
                                }
                                EditorUtility.SetDirty(_cut);
                            }
        ...
        {
            if (NudgeRedrawDeferred() || _cropBuilding)
                return;
            ...
            VoxelBlueprintPartBoxCutUtil.CountOwners(_cut, _counts, out _leftover);
        }
        ...
        int EditPartIndex()
        {
            ...
            if (!_cut.IsArmMirrorDest(label))
                return _selected;
            int mate = _cut.IndexOf(VoxelBlueprintPartBoxCut.ArmMate(label));
            return mate >= 0 ? mate : _selected;
        }

        string PartButtonLabel(int i)
        {
            string label = _cut.boxes[i].label;
            if (_cut.IsArmMirrorDest(label))
                return label + " 镜像";
            return label;
        }
        ...
        void DrawBoxes(PreviewRenderUtility preview)
        {
            ...
            VoxelBlueprintPartBoxCutGizmo.DrawBoxes(
        ...
        void DrawCropAxes(PreviewRenderUtility preview)
        {
            ...
            if (_cut == null || _cube == null || _gizmoMaterial == null)
                return;
            VoxelBlueprintPartBoxCut.Box box = _cut.boxes[EditPartIndex()];
            box.EnsureOriented();
            Quaternion rot = box.Rotation;
            if (EditPartIndex() != _selected)
            {
                Vector3 r = rot * Vector3.right; r.x = -r.x;
                Vector3 u = rot * Vector3.up; u.x = -u.x;
                Vector3 f = rot * Vector3.forward; f.x = -f.x;
                if (f.sqrMagnitude > 1e-8f && u.sqrMagnitude > 1e-8f)
        ...
        bool HandlePreview(Rect rect)
        {
            ...
            {
                VoxelBlueprintPartBoxCut.Box box = _cut.boxes[EditPartIndex()];
                bool changed = VoxelBlueprintPartBoxCutGizmo.TryHandle(
            ...
            int other = VoxelBlueprintPartBoxCutGizmo.HitOtherBox(rect, _preview.camera, _cut, _selected, gridCenter);
            Event e = Event.current;
        ...
        bool HandleWasd()
        {
            ...
            if (ViewingLeftover)
                return false;
            VoxelBlueprintPartBoxCut.Box box = _cut.boxes[_selected];
            Undo.RecordObject(_cut, "Nudge Part Box");
            int edit = EditPartIndex();
            box = _cut.boxes[edit];
            if (_tool == VoxelBlueprintPartBoxTool.Scale)
                VoxelBlueprintPartBoxCutGizmo.NudgeScale(ref box, step, e.shift ? -1f : 1f);
            else
                VoxelBlueprintPartBoxCutGizmo.NudgeMove(ref box, step.normalized);
            _cut.boxes[edit] = box;
            EditorUtility.SetDirty(_cut);
            ScheduleNudgeRedraw();
            e.Use();
        ...
        void BeginCropBuild()
        {
            ...
            VoxelBlueprintPartBoxCut.Box box = default;
            if (!_buildLeftover)
            {
            ...
            _buildDest = !_buildLeftover && _cut.IsArmMirrorDest(_buildLabel);
            _buildMate = _buildLeftover ? null : VoxelBlueprintPartBoxCut.ArmMate(_buildLabel);
            _cropBuilding = true;
            _scanX = 0;
            _scanY = 0;
            _scanZ = 0;
        ...
        void AccumulateCropCell(VoxelBlueprintAsset source, VoxelBlueprintAsset.Cell cell, int x, int y, int z)
        {
            ...
            if (_buildLeftover)
            {
                if (!VoxelBlueprintPartBoxCutUtil.CountsAsSliceLeftover(_cut, x, y, z))
                    return;
        ...
        bool CropCellBelongsToSelected(int x, int y, int z, string selectedLabel, bool dest)
        {
            ...
            if (dest)
            {
                if (owner == selectedLabel)
                    return true;
                return string.IsNullOrEmpty(owner) || owner == "Torso" || owner == "Hips";
            }
        ...
        {
            string label = _cut.boxes[_selected].label;
            bool dest = _cut.IsArmMirrorDest(label);
            if (_pickPaint)
                return !CropCellBelongsToSelected(cell.x, cell.y, cell.z, label, dest);
        ...
        static bool HasMarks(List<VoxelBlueprintPartBoxCut.RejectedCells> list)
        {
            if (list == null)
                return false;
            for (int i = 0; i < list.Count; i++)
            {
                VoxelBlueprintPartBoxCut.RejectedCells entry = list[i];
        ...
        static void ClearMarks(List<VoxelBlueprintPartBoxCut.RejectedCells> list)
        {
            if (list == null)
                return;
            for (int i = 0; i < list.Count; i++)
        ...
        static PaintSnap ClonePaint(VoxelBlueprintPartBoxCut cut)
        {
            cut.EnsureRejects();
            cut.EnsureClaims();
        ...
        static List<VoxelBlueprintPartBoxCut.RejectedCells> CloneMarks(List<VoxelBlueprintPartBoxCut.RejectedCells> srcList)
        {
            var copy = new List<VoxelBlueprintPartBoxCut.RejectedCells>(srcList.Count);
            for (int i = 0; i < srcList.Count; i++)
            {
                VoxelBlueprintPartBoxCut.RejectedCells src = srcList[i];
                var cells = new List<Vector3Int>();
                if (src != null && src.cells != null)
                    cells.AddRange(src.cells);
                copy.Add(new VoxelBlueprintPartBoxCut.RejectedCells
                {
        ...
        static void RestorePaint(VoxelBlueprintPartBoxCut cut, PaintSnap snapshot)
        {
            cut.EnsureRejects();
            cut.EnsureClaims();
            if (snapshot == null)
        ...
        static void RestoreMarks(
            List<VoxelBlueprintPartBoxCut.RejectedCells> dstList,
            List<VoxelBlueprintPartBoxCut.RejectedCells> snapshot)
        {
            if (snapshot == null)
                return;
            for (int s = 0; s < snapshot.Count; s++)
            {
                VoxelBlueprintPartBoxCut.RejectedCells src = snapshot[s];
                if (src == null || string.IsNullOrEmpty(src.label))
                    continue;
                for (int r = 0; r < dstList.Count; r++)
                {
                    VoxelBlueprintPartBoxCut.RejectedCells dst = dstList[r];
